...
public class Principal extends Pantalla implements ActionListener{
	...
	private static final int PRECIO_CAMA = 100;
	   ...
	   // Botones de panel logeo
		...
		//boton entrar Pagos
			if(e.getSource()==btnPagos) {
				...
				btnFacturarPagos.setEnabled(false);
			}
		...
		//boton volver a principal
			...
			if(e.getSource() == btnFacturarPagos){
				facturarPago();
			}
			...
			if (aInternamiento.buscarPorCodigo(codigoInternamiento)==-1) {// NO EXISTE CODIGO
				...
				if("".equals(DniPaciente)) {
					...
					Internamiento internamiento = new Internamiento(codigoInternamiento,
						 ...
						 actualizarEstadoCama(camaAsignada,Constantes.CAMA_OCUPADA);
	...
	private void actualizarEstadoCama(int numeroCama, int estado) {
	...
	private void buscarDniPacienteAtencion () {
		System.out.println("Principal buscarDniPacienteAtencion");
	...
	private void buscarDniPacientePago () {
		System.out.println("Principal buscarDniPacientePago");
			String numDniPaciente = txtDniPacientePagos.getText();	
				
			Paciente paciente = aPacientes.obtenerPacientePorDni(""+numDniPaciente);
				
				if (paciente!=null) {
					txtNombrePacientePago.setText(""+paciente.getNombresPaciente());

					Internamiento internamiento =
							aInternamiento.buscarPorCodigoPaciente(paciente.getNumPaciente());
					
					Atencion atencion =
							aAtencion.buscarPorCodigoPaciente(paciente.getNumPaciente());
					
					if(Constantes.INTERNAMIENTO_ALOJADO == internamiento.getEstado()){
						btnFacturarPagos.setEnabled(true);
						listadoPago(internamiento,atencion);
					}else{
						mensajeAlerta("El Paciente ya pago!");
					}
	...
	private void listadoPago(Internamiento internamiento,Atencion atencion) {
		System.out.println("entro listadoPago");
		try{
				
			dtmPagos.setRowCount(0);
			String fechaHoy = utilitario.obtenerFechaActualddMMyyyy();
			int cantidadDias = utilitario.restarFechas(internamiento.getFechaIngreso(), fechaHoy);
			
			double importe = PRECIO_CAMA * cantidadDias;
			
			dtmPagos.addRow(new Object[] {cantidadDias,"Dias","Internamiento",PRECIO_CAMA,importe});

			dtmPagos.addRow(new Object[] {0,"Unidades","Total Medicina",0,atencion.getTotalPagar()});
			
			double totalSinIGV = importe + atencion.getTotalPagar();
			double igv = 0.19 * totalSinIGV; 
			double total = totalSinIGV + igv;
			txtSubTotalPagos.setText(""+totalSinIGV);
			txtIGVPagos.setText(""+igv);
	...
	private void facturarPago() {
		System.out.println("Principal facturarPago");
		String numDniPaciente = txtDniPacientePagos.getText();	
		...
		Internamiento internamiento =
		 ...
		 actualizarEstadoCama(internamiento.getNumeroCama(),Constantes.CAMA_LIBRE);
		 
		 actualizarEstadoInternamiento(internamiento);
		 
		 Atencion atencion =
					aAtencion.buscarPorCodigoPaciente(paciente.getNumPaciente());
		 
		 actualizarEstadoAtencion(atencion);
		 limpiarCajasPago();
		 mensaje("Pagado Correctamente!");
		
	}
	
	private void actualizarEstadoInternamiento(Internamiento internamiento){
		//OBTENGO LA POSICION DE LA Internamiento
		 int pos = aInternamiento.buscarPorCodigo(internamiento.getCodigoInternamiento());
		 internamiento.setEstado(Constantes.INTERNAMIENTO_PAGADO);
		 internamiento.setFechaSalida(Utilitario.obtenerFechaActualddMMyyyy());
		 internamiento.setHoraSalida(Utilitario.obtenerHora());
		 aInternamiento.modificarEstado(pos, internamiento);
	}
	
	private void actualizarEstadoAtencion(Atencion atencion){
		//OBTENGO LA POSICION DE LA Atencion
		 int pos = aAtencion.buscarPorCodigo(atencion.getCodigoAtencion());
		 atencion.setEstado(Constantes.ATENCION_PAGADA);
		 aAtencion.modificarEstado(pos, atencion);
	}
	...
	String obtenerDniPago () {
		return txtDniPacientePagos.getText();
	}
